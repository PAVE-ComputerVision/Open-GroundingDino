...
def translate_bounding_box(bbox, crop_bbox):
    ...
    """
    For example:
        [1232, 134, 512, 198] is not acceptable due to the 1232
        [501, 181, 512, 245] is acceptable because all are less than 512

    the formay we are going for is:
    [x1, y1, x2, y2] where each value is <= 512

    Problem happens only when x1 or x2 > 512
    """
    x1, y1, x2, y2 = bbox
    ...
    #    assert new_x2 > new_x1, f"x2 must be > x1, however x1: {new_x1}, x2: {new_x2}"
    #    new_x1 = 512 
...
def is_partially_inside(bbox1, bbox2):
    """
    Check if bbox1 is completely inside bbox2.
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, ori_samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            image_tensor = samples.tensors
            ori_tensor = ori_samples
            crops, ori_crops, crop_bboxes = create_crops_v3(image_tensor, ori_tensor, car_bbox_resp)
            ...
            for crop, crop_bbox in zip(crops, crop_bboxes):
                ...
                for i, dmg_bbox in enumerate(scaled_dmg_bboxes):
                    if is_inside(dmg_bbox, crop_bbox):
                        ...
                        if min(new_bbox) < 0:
                            print('dmg bbox', dmg_bbox)
                            print('relative_dmg_bbox', relative_dmg_bbox)
                            print('Skipping because x1 > x2 or y1 > y2 in dmg bbox')
                            continue
            ...
            for i in range(len(crops)):
                if len(crop_targets[i]['boxes']) != 0:
                    ...
                    #crop_img = draw_bounding_boxes(ori_crops[i], scaled_dmg_tnsr, width=3, colors = "red")
